...
func (fs *Fs) version(
	...
	// TODO: Well we should defintiely be speaking fucking 9p2000.l
...
func (fs *Fs) versionPost(msize uint32, dotu bool) { fs.Dotu = dotu }

func (fs *Fs) auth(afid uint32, uname string, aname string, unamenum uint32) (*Qid, error) {

	// We don't support any authentication.
	return nil, Enotimpl
}

func (fs *Fs) attach(fid uint32, afid *Fid, uname string, aname string, unamenum uint32) (*Qid, error) {
	// As per above,
...
func (fs *Fs) walk(fid *Fid, newfid uint32, names []string) ([]Qid, error) {
	// Do the actual walk.
...
func (fs *Fs) open(fid *Fid, mode uint8) (*Qid, uint32, error) {
	// Already opened?
	if fid.Opened {
		return nil, 0, Eopen
	}
...
func (fs *Fs) openPost(fid *Fid, mode uint8) {
	fid.Omode = mode
	fid.Opened = true
}

func (fs *Fs) create(fid *Fid, name string, perm uint32, mode uint8, ext string) (*File, *Qid, uint32, error) {
	// Already opened?
	if fid.Opened {
...
func (fs *Fs) createPost(fid *Fid, new_file *File, name string, mode uint8) {
	// Swap out the files.
	fid.file.DecRef(fs, fid.Path)
	fid.file = new_file
...
func (fs *Fs) createFail(fid *Fid, file *File, name string, mode uint8) {
	if file != nil {
		// Drop the new file.
...
func (fs *Fs) readDir(fid *Fid, offset int64, length int) ([]*Dir, error) {
...
func (fs *Fs) readFile(fid *Fid, offset int64, length int) (int, error) {
...
func (fs *Fs) readDirPost(fid *Fid, count uint32, entries int) {
	if fid.file.Qid.Type&QTDIR != 0 {
...
func (fs *Fs) writeFile(fid *Fid, offset int64, count int) (int, error) {
	err := fid.file.lockWrite(fs)
	return fid.file.write_fd, err
}

func (fs *Fs) readFilePost(fid *Fid, count uint32)  { fid.file.unlock() }
func (fs *Fs) readFileFail(fid *Fid, count uint32)  { fid.file.unlock() }
func (fs *Fs) writeFilePost(fid *Fid, count uint32) { fid.file.unlock() }
func (fs *Fs) writeFileFail(fid *Fid, count uint32) { fid.file.unlock() }
func (fs *Fs) clunk(fid *Fid) error                 { return nil }
...
func (fs *Fs) wstatFail(fid *Fid, cur *Dir, next *Dir) { fid.file.unlock() }
