...
type PciBus struct {
	...
	MemoryRegion kvm.Pointer `json:"reserved"`
...
func NewPciBus(info *DeviceInfo) (Device, error) {
	...
	bus.Hole.Size = uint64(kvm.IOApic() - bus.Hole.Start)

	// Return our bus and device.
	return bus, bus.init(info)
}

func (pcibus *PciBus) Attach(vm *kvm.VirtualMachine, model *Model) error {
...
func (pcidevice *PciDevice) RebuildBars() {
	...
	for i := uint(0); i < pcidevice.PciBarCount; i += 1 {
		...
		region := MemoryRegion{
			kvm.Pointer(baraddr & ^uint32(0xf)),
			uint64(barsize)}
...
func (pcidevice *PciDevice) Attach(vm *kvm.VirtualMachine, model *Model) error {
	...
	pcidevice.std_interrupt = func() error {
		vm.Interrupt(kvm.Irq(1), true)
		vm.Interrupt(kvm.Irq(1), false)
		return nil
	}
