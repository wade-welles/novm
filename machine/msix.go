...
type MsiXDevice struct {
	...
	*MsiXConf
	...
	msi_interrupt func(addr kvm.Pointer, data uint32) error
}
...
func NewMsiXDevice(
	...
	if bar_size < kvm.PageSize {
		bar_size = kvm.PageSize
	}
...
func (msix *MsiXDevice) Attach(vm *kvm.VirtualMachine, model *Model) error {
	...
	msix.msi_interrupt = func(addr kvm.Pointer, data uint32) error {
...
func (msix *MsiXDevice) SendInterrupt(vector int) error {
	...
	return msix.msi_interrupt(kvm.Pointer(paddr), uint32(data))
}
